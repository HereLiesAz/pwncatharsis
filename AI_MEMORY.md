...
- **DECISION**: After multiple attempts to use `pwncat-cs` and `pwncat-ci` resulted in dependency
  and build failures with Chaquopy, the project will use a simple, stable architecture where the
  source code for the `pwncat` script is bundled locally as `app/src/main/python/pwncat.py`. The
  `session_manager.py` script will import this local module directly. This approach removes all
  external network dependencies for the core backend and has proven to be the most robust solution.
  All previous dependency and architecture entries are now obsolete.

### ENTRY 19: NEW FEATURE SCOPE - NETWORK RECON & PAYLOADS

- **DATE**: 2025-06-21
- **STATUS**: Active
- **DECISION**: The application will be expanded with a suite of new features focused on network
  reconnaissance and more advanced payload generation.
- **DETAILS**:
    - **1. Reconnaissance Module Consolidation:**
        - All network analysis features will be consolidated under the "Recon" tab, rather than a
          separate "Network" tab.
        - This module will include:
            - **Wi-Fi Scanning:** Scan for and display local Wi-Fi networks, including SSID and
              signal strength. This will require native Android API access and location permissions.
            - **IP/Profile Management:** Allow the user to get their current IP address and save
              static IP profiles (e.g., "Home VPN," "Work Lab") for easy reuse in payload
              generators.
            - **Network Host Discovery:** Perform scans on the local network to discover live hosts,
              identifying their IP, MAC address, and attempting to fingerprint the OS and device
              type.
            - **Point-and-Click Port Scanning:** Integrate a simple UI to run and save port scans
              against discovered hosts.
    - **2. Advanced Payload Generation:**
        - **Website Cloning:** Integrate the functionality of `makephish` to clone a website for
          phishing campaigns.
        - **Payload Injection:** Embed the reverse shell commands generated by the app into the
          cloned websites.
    - **3. Data Persistence and Management:**
        - All user-generated data and scan results must be saved locally on the device. This
          includes:
            - Saved IP and Wi-Fi profiles.
            - Saved network scan results (discovered hosts and their properties).
            - Saved port scan results for each host.
            - Generated reverse shell payloads.
            - Custom names for shells/sessions.
    - **4. External Intelligence Integration (Shodan):**
        - Integrate the Shodan API to enrich discovered public IP addresses with external data.
        - The `SettingsScreen` will be updated to allow the user to enter their Shodan API key.
    - **5. Usability Enhancements:**
        - **Custom Session Naming:** Allow the user to assign a custom alias to each active session
          for better organization.
        - **Persistent Shells:** All generated reverse shell payloads must be designed for
          persistence, leveraging the reconnection capabilities of the `pwncat` backend.
- **CREDITS TO BE MAINTAINED IN README:**
    - `cytopia/pwncat`
